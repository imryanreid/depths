...
If a shared component needs behaviour specific to this tool, give it a **prop**—never a branch on which tool is running. `src/shared/` must never import from
...
**5. The edges.** `--edge-*` borders ship in BOTH modes—ink hairlines on
light, white on dark, the same strength curve (two-mode since 2026-09-06;
they began dark-only with transparent light values)—so
`border: var(--edge-raised)` can be applied unconditionally without the theme
flip shifting layout. Emitting them only under `.dark` would reintroduce
exactly that layout shift. The toggle lives in the scale section's header but
is real state: the `k` param, shipped in every export.
...
`depths.studio` is registered and `www.depths.studio` is the canonical host—`SITE_URL`, `index.html` (canonical, `og:url`, JSON-LD), `public/robots.txt`,
