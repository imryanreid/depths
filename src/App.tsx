      ...
      controls={
        <div className="flex flex-wrap items-end gap-x-8 gap-y-6">
          ...
          <div>
            ...
            <Segmented
              options={[
                { id: "s", label: "Shadows", title: "Boosted alphas only" },
                { id: "sb", label: "+ Edges", title: "Boosted alphas plus hairline borders" },
              ]}
              value={config.dark}
              onChange={(v) => patch({ dark: v as "s" | "sb" })}
              layoutId="dark-pill"
              ariaLabel="Dark-mode strategy"
            />
          </div>
 ...
 * uses. Edges collapse to transparent when the dark strategy is shadows-only,
 * so the previews honestly show what that strategy costs.
 */
function previewStyle(scale: ResolvedScale, tokens: ResolvedToken[]): string {
  ...
  for (const lv of scale.levels) {
    ...
    light.push(`  --depths-edge-${lv.level}: ${edgeValue(lv.level, "light")};`)
    dark.push(
      `  --depths-edge-${lv.level}: ${edges ? edgeValue(lv.level, "dark") : "1px solid transparent"};`,
    )
  }
  ...
  for (const t of tokens) {
    ...
    if (t.effectiveLevel !== 0) {
      light.push(`  --edge-${t.id}: ${t.edgeLight};`)
      dark.push(`  --edge-${t.id}: ${edges ? t.edgeDark : "1px solid transparent"};`)
    }
  }
  return `:root {\n${light.join("\n")}\n}\n.dark {\n${dark.join("\n")}\n}`
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <span
          aria-hidden="true"
          className="border-line h-9 w-9 shrink-0 rounded-md border"
