 ...
 *
 * Edges, when on, ship in BOTH modes—ink hairlines on light, white on dark,
 * the same strength curve—so the same `border: var(--edge-raised)` works
 * whichever theme is active and the theme flip never shifts layout.
 */
export function toCss(scale: ResolvedScale): string {
  ...
  if (edges) {
    ...
    for (const t of tokens) {
      if (t.effectiveLevel === 0 || t.effectiveLevel === "inset") continue
...
export function toTailwind(scale: ResolvedScale): string {
  ...
  if (edges) {
    lines.push(
      ...
      "   Ink on light, white on dark; the theme flip never shifts layout. */",
      ":root {",
    )
    for (const t of tokens) {
      if (t.effectiveLevel === 0 || t.effectiveLevel === "inset") continue
...
export function toNative(scale: ResolvedScale, url: string): string {
  ...
  if (edges) {
    lines.push("")
    lines.push("  /// Hairline opacity for both modes—stroke your own shape with it,")
    lines.push("  /// black in light and white in dark, the same strength curve.")
    lines.push("  func edgeOpacity() -> Double {")
    lines.push("    switch self {")
    for (const t of tokens) {
    ...
    lines.push("")
    lines.push(
      "  /// The edge color for a scheme: `.stroke(token.edgeColor(scheme), lineWidth: 1)`.",
    )
    lines.push("  func edgeColor(_ scheme: ColorScheme) -> Color {")
    lines.push("    (scheme == .dark ? Color.white : Color.black).opacity(edgeOpacity())")
    lines.push("  }")
  }
  lines.push("}")
  lines.push("")
...
export function toAgentMarkdown(
  ...
  lines.push(
    ...
    `light source, mapped to semantic tokens. Preset: **${preset.name}**${presetEdited(scale).length ? ` (edited: ${presetEdited(scale).join(", ")})` : ""}—${preset.blurb}`,
  )
  lines.push("")
  ...
  lines.push("")
  lines.push(
    "Paste as-is. Apply with `box-shadow: var(--shadow-raised)`, and",
    "`border: var(--edge-raised)` where the variable exists—the hairline is ink",
    "on light and white on dark, and the theme flip never shifts layout.",
  )
  lines.push("")
  lines.push("```css")
...
/** Which curve fields differ from the chosen preset—so a reader can't
 *  honestly call an edited scale "the Soft preset". The page badges it;
 *  the files say it too. */
function presetEdited(scale: ResolvedScale): string[] {
  const params = PRESETS[scale.config.presetId].params
  return (Object.keys(params) as (keyof typeof params)[]).filter(
    (k) => scale.config[k] !== params[k],
  )
}
...
3. Apply border: var(--edge-*) alongside each shadow where the variable exists (ink on light, white on dark), and in dark mode prefer raising surface color over inventing stronger shadows.`
}
